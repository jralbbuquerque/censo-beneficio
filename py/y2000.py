...
def giniEstados(estados):
    ...
    for k in estados:
        ...
        results = results.append( {'COD': cod_uf, 'UF': str.upper(k), 'Gini_RD': g_RD[0],
                                   'Gini_RP': g_RP[0], 'RendaMedia': round(renda, 2),
...
def giniCidades(estados):
    ...
    for k in estados:
        ...
        for i in cidades['cidade']:
            ...
            results = results.append( {'COD': i, 'UF': str.upper(uf), 'Gini_RD': g_RD[0], 
                                       'Gini_RP': g_RP[0], 'RendaMedia': round(renda, 2),
                                       'RendaMedia2010': round(renda2010, 2)}, ignore_index=True )
...
def beneficioCidades(estados):
    ...
    for k in estados: 
        ...
        for i in cidades['cidade']:
            ...
            results = results.append( {'UF': str.upper(uf), 'COD': i, 'Renda(Total)': RT,
                                      'Renda(Beneficios)': RB, 'Total(%)': RB/RT, 'Beneficio(>Sal)': RB_nSAL,
